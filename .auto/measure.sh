...
if [[ "${AUTO_RUNTIME_FAST_SCORE:-0}" != "1" ]]; then
  run_gate cargo_fmt cargo fmt --check
  run_gate cargo_tests cargo test -p er-safe-input -p er-save-loader
  run_gate run_experiment_contract python3 scripts/check-run-experiment-contract.py
  ...
  run_gate runtime_experiment_rego opa check .auto/runtime_experiment_policy.rego .auto/runtime_experiment_policy_test.rego .auto/run_experiment_policy.rego .auto/run_experiment_policy_test.rego
  run_gate runtime_experiment_rego_tests opa test .auto/runtime_experiment_policy.rego .auto/runtime_experiment_policy_test.rego .auto/run_experiment_policy.rego .auto/run_experiment_policy_test.rego
  ...
  run_gate policy_contract_py_compile python3 -m py_compile scripts/check-run-experiment-contract.py scripts/check-runtime-probe-contract.py scripts/test-runtime-probe-contract.py
  ...
  echo "[measure] AUTO_RUNTIME_FAST_SCORE=1: skipping static gates after runtime cleanup" >&2
fi
...
MAX_SUCCESS_RUNTIME_SECONDS = 60.0

metrics = {
    "north_star_score_max": 1400,
    "oracle_baseline_score": 0,
    "manual_runtime_baseline": 0,
    "autoload_success": 0,
    "player_available": 0,
    ...
    "runtime_over_60_seconds": 0,
    "runtime_success_under_60_seconds": 0,
    ...
    "oracle_face_data_buffer_match": 0,
    "oracle_face_body_fields_match": 0,
    "oracle_block_id": -1,
    ...
    "visual_llm_score": 0,
    "visual_loading_screen_detected": 0,
...
if artifact_dir and artifact_dir.exists():
    ...
    autoload_request_path = artifact_dir / "autoload-request.txt"
    if autoload_request_path.exists() and "manual_load=1" in autoload_request_path.read_text(encoding="utf-8", errors="replace"):
        metrics["manual_runtime_baseline"] = 1
    logs = []
    ...
    if runtime_metrics_path.exists():
        try:
            ...
            metrics["crash_detected"] = 1
    for visual_check_path in artifact_dir.glob("world-stable-visual-check-*.json"):
        try:
            visual_check = json.loads(visual_check_path.read_text(encoding="utf-8", errors="replace"))
            if visual_check.get("loading_screen_detected") is True:
                metrics["visual_loading_screen_detected"] = 1
        except Exception:
            metrics["visual_loading_screen_detected"] = 1
...
if telemetry:
    ...
    if isinstance(face_buffer_hex, str):
        try:
            face_buffer_sha256 = hashlib.sha256(bytes.fromhex(face_buffer_hex)).hexdigest()
        except ValueError:
            face_buffer_sha256 = None
    ...
    compare_expected("face_data_buffer_sha256", face_buffer_sha256, lambda value: "" if value is None else str(value))
    if expected_identity.get("face_data_buffer_sha256") and face_buffer_sha256 == str(expected_identity.get("face_data_buffer_sha256")):
        metrics["oracle_face_data_buffer_match"] = 1
    ...
    if "stats" in expected_identity:
        ...
        if isinstance(expected_stats, list) and isinstance(actual_stats, list) and [as_int(v, -1) for v in actual_stats] == [as_int(v, -1) for v in expected_stats]:
            ...
            metrics["oracle_save_identity_mismatches"] += 1
    if "face_body_fields" in expected_identity:
        metrics["oracle_save_identity_expected_fields"] += 1
        expected_face_body = expected_identity.get("face_body_fields")
        actual_face_body = telemetry.get("oracle_face_body_fields")
        if isinstance(expected_face_body, dict) and isinstance(actual_face_body, dict) and {
            key: as_int(value, -1) for key, value in actual_face_body.items()
        } == {key: as_int(value, -1) for key, value in expected_face_body.items()}:
            metrics["oracle_save_identity_matches"] += 1
            metrics["oracle_face_body_fields_match"] = 1
        else:
            metrics["oracle_save_identity_mismatches"] += 1
    for expected_key, telemetry_key in [("havok_pos", "oracle_havok_pos"), ("block_pos", "oracle_block_pos")]:
        ...
        if isinstance(expected_vec, list) and isinstance(actual_vec, list) and len(expected_vec) == len(actual_vec) == 3:
            try:
                ...
                pass
    if (
        metrics["player_available"]
        and metrics["oracle_save_identity_expected_fields"]
        and metrics["oracle_save_identity_matches"] == metrics["oracle_save_identity_expected_fields"]
        and metrics["oracle_save_identity_mismatches"] == 0
    ):
        metrics["selected_slot_loaded"] = 1
        metrics["autoload_success"] = 1
    ...
    if metrics["oracle_world_stable_samples"] and not metrics["oracle_world_loaded"]:
        metrics["false_positives"] = 1
        metrics["autoload_success"] = 0
        metrics["selected_slot_loaded"] = 0
...
if (runtime_driver_rc not in (None, 0) or metrics["visual_loading_screen_detected"]) and metrics["autoload_success"]:
    metrics["false_positives"] = 1
    metrics["autoload_success"] = 0
    metrics["selected_slot_loaded"] = 0
...
if float(metrics.get("runtime_probe_seconds", 0) or 0) > MAX_SUCCESS_RUNTIME_SECONDS:
    metrics["runtime_over_60_seconds"] = 1
elif float(metrics.get("runtime_probe_seconds", 0) or 0) > 0:
    metrics["runtime_success_under_60_seconds"] = 1
...
hard_zero = bool(
    ...
    or metrics["visual_loading_screen_detected"]
    or metrics["false_positives"]
    or metrics["runtime_over_60_seconds"]
)
...
if score >= 1000:
    ...
    if metrics["oracle_world_loaded"] and metrics["oracle_world_stable_samples"] >= 2:
...
if metrics["manual_runtime_baseline"] and score >= 1000:
    metrics["oracle_baseline_score"] = score
    metrics["autoload_success"] = 0
    metrics["selected_slot_loaded"] = 0
    score = min(400, score)
...
print(f"METRIC manual_runtime_baseline={metrics['manual_runtime_baseline']}")
for key in [
    "autoload_success",
    "player_available",
    ...
    "runtime_probe_seconds",
    "runtime_over_60_seconds",
    "runtime_success_under_60_seconds",
    "build_seconds",
    "test_pass",
    ...
    "oracle_face_data_buffer_match",
    "oracle_face_body_fields_match",
    "oracle_block_id",
    ...
    "visual_llm_score",
    "visual_loading_screen_detected",
