...
def read_module_tree(root_file: Path, module_dir: Path | None = None) -> str:
    ...
    These metric checks started as string checks over one large source file. The codebase is now
    split across `foo.rs` + `foo/` include trees (recursively, e.g. experiments/startup_hooks/),
    so the metric must inspect the whole logical module instead of accidentally scoring refactors
    as feature removal. Mirrors read_module_tree in scripts/check-autoload-happy-path.py.
    """
    parts: list[str] = []
    if root_file.exists():
        parts.append(root_file.read_text(encoding='utf-8', errors='replace'))
    if module_dir is not None and module_dir.is_dir():
        ...
        parts.extend(p.read_text(encoding='utf-8', errors='replace') for p in files)
    return '\n'.join(parts)
...
if 'OWN_STEPPER_SETTLE_CALLS' not in check or 'NATIVE_LOAD_SETTLE_FRAMES' not in check:
    ...
    autoload_static_failures += 1
if (
    'suppressed MessageBoxDialog build scope=' not in exp
    or 'MSGBOX_LAST_ARG_RDX.store' not in exp
    or 'dump_msgbox_spec' not in exp
    or 'msgbox-skip #' not in exp
):
    message = 'product-mode MessageBoxDialog suppression can hide popup builder attempts from debug evidence'
    legacy_failures.append(message)
    autoload_static_failures += 1
if 'product-mode MessageBoxDialog suppression must log specific build args/spec/caller evidence without publishing ambiguous build-count telemetry' not in check:
    legacy_failures.append('check-autoload-happy-path does not enforce spec-logged product-mode MessageBoxDialog suppression')
    autoload_static_failures += 1
if (
...
if 'quality gates must include skip-safe native Continue/MenuWindowJob/MenuMemberFuncJob/result-consumer static byte-window validation' not in check:
    ...
    autoload_static_failures += 1
if (
    # 2026-07-19 reconciliation (2b91ad9): the product design no longer force-calls open_menu from
    # the game task; the enforced source comment is now the "do NOT call" preservation note. The two
    # forbidden Loop/TextFadeout timing-coupling patterns remain the enforced anti-patterns.
    'do NOT call TitleTopDialog::open_menu from this game-task' not in exp
...
# 2026-07-27 blocking-modal reconciliation: telemetry no longer publishes ambiguous historical
# builder-count oracles (oracle_msgbox_total_builds/any_seen). The run-stopping semaphore is the
# ACTIVE blocking-modal state; specific builds are logged via dump_msgbox_spec debug evidence.
for token in [
    'oracle_blocking_modal_present',
]:
    if token not in telemetry_src:
        messagebox_dialog_failures.append(f'telemetry missing active blocking-modal oracle token {token}')
for token in [
...
if rt_root.exists():
    ...
    for d in candidates:
        ...
        for name in ['readiness-result.json', 'max-oracle-result.json', 'telemetry.json', 'er-effects-telemetry.json']:
            ...
            if re.search(r'"reason"\s*:\s*"native_messagebox_dialog_detected"', raw) or re.search(r'"oracle_blocking_modal_present"\s*:\s*true', raw):
                messagebox_by_dir[d.name] = [
                    f'runtime artifact {d.name} observed an active blocking native CS::MessageBoxDialog; product proof requires zero blocking MessageBoxDialog popups'
                ]
            if (
...
part_a_common = (
    ...
    # Lifecycle install/call_once seams moved from lib.rs into experiments/lifecycle.rs
    # (experiment lifecycle refactor); search the combined runtime corpus like the checker does.
    and 'START_TITLE_NATIVE_MENU_VISUAL_SUPPRESS.call_once' in code
    and 'install_title_native_menu_visual_suppression_hook' in code
    and code.find('START_TITLE_NATIVE_MENU_VISUAL_SUPPRESS.call_once') < code.find('START_MENU_WINDOW_LATCH.call_once')
...
part_a_render_only = (
    ...
    and 'START_TITLE_NATIVE_MENU_VISUAL_RENDER_SUPPRESS.call_once' in code
    and 'install_title_native_menu_visual_render_suppression_hook' in code
...
part_b_pressstart_suppression = (
    ...
    and 'install_title_gfx_value_set_visible_hook' in code
    and 'install_title_scene_obj_proxy_named_child_bind_hook' in code
...
profile_select_canvas_installed = (
    'START_TITLE_CUSTOM_COVER_RUN.call_once' in code
    or 'install_title_custom_cover_run_hook' in code
    or 'build_profile_select_cover_job(base, rdx, r8' in code
)
profile_select_one_tick_cropped = (
    # The one-tick deferral is now the TITLE_PROFILE_FACE_LAST_VALUE-gated deferred transform in
    # the custom-cover run hook (TITLE_CUSTOM_COVER_RUN_CALLS-counted), not a current_calls check.
    'deferred transform after custom cover' in code
...
portrait_overlay_cover_observable = (
    ...
    and '&& !product_autoload_enabled()' in code
)
