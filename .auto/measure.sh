...
if 'telemetry/watcher oracle must expose passive native submit/result-handler/action-builder/wrapper-builder/action-insert hit counts, wrapper/update-RVA proof, same-result proof, and chain stage' not in check:
    ...
    autoload_static_failures += 1
if 'native static checker must pin wrapper-builder ABI, ToS status predicate/setter/caller ABI, and inner finalize edge' not in check:
    legacy_failures.append('check-autoload-happy-path does not enforce native wrapper-builder/ToS predicate+setter+caller ABI static check')
    autoload_static_failures += 1
