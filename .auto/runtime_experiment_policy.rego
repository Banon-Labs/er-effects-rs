...
deny contains message if {
	not valid_timeout
	message := "runtime probe rejected: timeout_seconds must be present, numeric, greater than 0, and no more than 60"
}

deny contains message if {
	input.explicit_opt_in
	not manual_event_driver_ready
	message := "runtime probe rejected: require measure runtime trigger, timeout_seconds<=60, scripts/er-readiness-watch.py, no-telemetry bootstrap failure, host_input=none, process/save teardown, no native title accept-gate mutation, and an approved launch mode"
}
