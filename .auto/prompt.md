...
- Kept run #14 confirmed the no-env auto-splash path at `fast_load_seconds=42.085`, `north_star_score=1400`; kept run #15 reduced live-dialog activation settle from 90 to 60 frames and scored `fast_load_seconds=40.716`, `north_star_score=1400`.
- Crash run #16 showed 30-frame live-dialog settle is too aggressive: it produced `fast_load_seconds=999`, `north_star_score=0`, `false_positives=1`, with visual/world-looking evidence but no selected-save/player correctness. Rerun #17 showed 45-frame settle is also unsafe: it looked like it loaded visually but failed the structured player/selected-save oracle. The fixed threshold is locked at 60 frames by `scripts/check-autoload-happy-path.py`; do not go below 60 without replacing it with a real readiness predicate and updating the guard.
- Discarded 15-frame initial title settle: structured save/player fields looked good, but the screenshot showed a modal saying OK returns to title menu and the character was not visible. Treat this as unsafe/non-controllable until the visual/player-presence mismatch is explained; do not mask it by auto-dismissing the modal after world load.
- Discarded 20-frame initial title settle and 150-frame PHASE_MENU_BUILD modal grace both reproduced the same unsafe screenshot: `Connection to Steam interrupted. Returning to title menu.` over world scenery with no visible character. Fixed initial settle must stay at 25 and fixed modal grace must stay at 180 unless a positive readiness predicate replaces the wait.
...
- Old source at `https://github.com/chozandrias76/er-skip-splash-screens` confirms the external DLL only did one patch: `start_of_jump_instruction = 0xB0C3ED`, expected `[0x74]` (`JZ`), replacement `[0x7F]` (`JG`). It supports the one BeginLogo-branch-flip interpretation and does not reveal a second splash patch.
