...
> **RUNTIME REDIRECTION (runs 302-303):** read-only runtime probes proved the SelectBot registry `[0x143d87360]` is LIVE in retail (`= 0x143b432a8`, stable at the title idle) — but the outer SimpleTitleStep menu system is **DORMANT** at the press-any-button screen: the pump-ran flag `[input_mgr 0x143d6b7b0 + 0x6b0]` stays 0 (set by BOTH outer MenuInit `0xb0a4a0` and MenuLoop `0xb0a5e0`), the inner TitleStep is parked at state 10 (MenuJobWait), and selection `+0x130` is never populated. **So SelectBot injection is a no-op pre-accept — the selection lane sits BEHIND the title-accept gate, not around it.** The highest-leverage target is now the native **title-accept / MenuJobWait(state 10) advance** with zero input (re-confirms runs 292-293). The SelectBot/menu decode below characterizes everything DOWNSTREAM of accept — valuable once accept is solved. See bd `selectbot-pump-dormant-pre-title-accept`. NEXT: reverse how inner MenuJobWait `0xb0d400` advances (owner+0xe0 menu job / title-accept input node `0x7ad1c0` / input manager `0x143d6b7b0`), and whether a bot-activation global starts the outer system earlier.
...
- 760: native selection injected at runtime and observed PARSED (`owner+0x130` set / `+0x128` non-empty) without forcing TitleStep state. **Producer side mapped (run 301):** registry root = global `0x143d87360`, accessor ctor `0xe7b650` binds named channel via `0x14011a360(obj+8, name, registry)`, reader vtable `0x142bfae68`; channels `0x142b5ea18` `"...MoveMapListStep"` and `0x142b60758` `"...MoveMapStep"`; second consumer `0xaf0710` (likely the real MoveMapListStep load handler) and registrar `0xaf5140`. **NEXT:** reverse `0x14011a360` for the registry entry layout + write/put API, determine the slot-9 PlayGame key value (`DD_DD_DD_DD` path), then inject into `0x143d87360` under `"CSEzSelectBot.MoveMapListStep"` and runtime-probe.
...
Frame-task rate (`runtime_frame_task_hz`) is NOT a hard zero gate (removed 2026-06-15 per user direction). The min-over-run conflates legitimate low-tick phases — cold boot, loading screens, and the actual save load we are trying to reach — with DLL-caused per-frame cost, so hard-zeroing on it would auto-discard a genuinely successful autoload. Keep it as a secondary observability metric (see below): a sustained low avg during the *title idle* phase still flags expensive per-frame DLL work (e.g. the title-owner VirtualQuery scan regression) and should be investigated and fixed, but it does not auto-discard a run.
