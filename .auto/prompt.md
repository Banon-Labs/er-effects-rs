...
- **north_star_score** (unitless, higher is better):
  ...
  - 960-minus-buttons: selected Seamless/ER save slot loads repeatably with deterministic in-process safe-input assistance, and structured evidence identifies why each pre-Continue input was needed (for example unsafe-shutdown warning, Seamless welcome, online warning, title accept, Continue) so the input path is state/menu-aware rather than fixed-count.
  - 900-ish medium tier: selected Seamless/ER save slot loads with in-process safe-input assistance, but inputs are still unexplained fixed pulses or the triggering menu conditions are unknown. Penalize by actual `simulated_button_presses_total`; do not reward high fixed budgets just because the run succeeds.
...
- Kept result `291a746` (building on `64d4d96`) reaches the safe-input-assisted tier on Steam/Proton/Seamless slot 9: DirectInput-gated in-process safe-input Confirm pulses (delayed until `IDirectInputDevice8::GetDeviceState` hook installs) clear title/dialog gating, DirectMenuLoad queues slot 9, telemetry reports `player_available=true`, `selected_slot_loaded=1`, `title_bootstrap_seen=1`, `native_request_consumed=1`, `save_safety_ok=1`, `er_process_teardown_ok=1`, `host_pointer_input_used=0`. Under the zero-button-aware scorer the current best is 944 because `simulated_confirm_presses=6` / `simulated_button_presses_total=6` and safe-input success scores `950 - simulated_button_presses_total`; 0 is reserved for the ideal native/menu-task path. Known-good reductions for the currently observed startup/menu stack: 12@30 ticks, 11@40, 10@50, 9@60, 8@75, and current 6@110. The required Confirm count is not globally deterministic: pre-Continue menus/popups vary, at least based on whether the prior shutdown was clean and triggers a warning popup. Treat fixed-count minima as current-stack evidence only, not a robust final strategy. Prefer state-gated/menu-aware Confirm pulses or native menu transitions that tolerate extra/missing popups, and measure actual pulses sent. Failures: 10/11@30 reached native consumption but not player availability; 20-tick spacing failed before native consumption; 25-tick spacing reached native consumption but not player availability; 5@135 and 5@135 with 8-frame holds reached native consumption but not player availability. Evidence includes `/home/banon/projects/er-effects-rs/target/smoke/autoload-runtime-20260613-182156` for current 6-confirm success.
