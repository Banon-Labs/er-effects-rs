...
## Current title-cover metric contract
- `05_010_ProfileSelect` cover telemetry (`oracle_title_custom_cover_profile_select_builds`, `oracle_title_custom_cover_run_calls`, `oracle_title_scaleform_bind_observer_profile00_bound`, and related SYSTEX/dummy-profile bind counters) is **native plumbing evidence**, not by itself product title-cover acceptance.
- `oracle_title_profile_cover_bound_to_logo_surface=true` is not required as the only possible future product route, because `05_001_Title_Logo` lacks dummy-profile/SYSTEX symbols; however, replacing it requires a new RAM-backed visible-cover oracle that ties the custom cover window/GFx display object to an active rendered title-safe surface after native title/PAB suppression.
- Do not score a ProfileSelect build/run/bind-only artifact as final product success. Final success still requires zero simulated input, zero messagebox/legal/server/save popups, native Continue/load/confirm edges, expected save identity, and world-loaded proof.
...
## What to Try Next
1. Build the missing RAM-backed visible-cover oracle; do **not** relax scoring. Current ProfileSelect semaphores are plumbing-only. The next keepable improvement should connect these three proofs:
   - **source**: active-screen slot/profile renderer -> `CSMenuProfModelRend` vtable (`base+0x2b80128`) -> `renderer+0xa8` `CSEzOffscreenRend` -> `offscreen+0x10` runtime texture resource cap, with `renderer+0x754/+0x755 == 0` and `renderer+0x9a8 == slot*2`;
   - **destination**: `TitleTopDialog+0xaa8` -> `TitleBackViewParts`, `+0x8` embedded `MenuResource("05_001_Title_Logo")`, `+0x70` `SceneObjProxy`, `+0x88` GFx/ScaleformValue handle;
   - **linkage/visibility**: classify `FUN_1407452c0` bind `owner` / `owner+0x60` and downstream resolver/apply path to a live title-safe rendered surface, not merely the `05_010_ProfileSelect` job.
2. The immediate code-facing seed is the new telemetry from this prep pass: `oracle_title_profile_source_slot0_*` and `oracle_title_profile_source_*_mask` are now exposed in `er-effects-telemetry.json` and watcher summaries. Use them only as source proof; add destination/owner classification before any product predicate flips true.
3. Best next static/runtime-instrumentation candidate: hook/read `FUN_140749290` (dump `0x140749290`, re-ground live RVA with `scripts/dump-deobf-shift.py` before adding a constant) to capture `param_1` MenuResource owner, `param_2` SceneObjProxy out, `param_1+0x60`, `param_1+0x48` name, and whether `param_2 == TitleBackViewParts+0x70` / `param_1 == TitleBackViewParts+0x8`.
...
5. After visible-cover oracle work, return to product ordering: current blocker is title accept/menu-open versus native save-data `ShowProgress` (`oracle_corrupted_save_seen_id=401106` when opened too early; waiting for profile active byte stalls). Preserve manual/native Banon load proof and zero-input/no-popup gates.
