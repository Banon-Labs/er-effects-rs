...
- Hudhook/fullscreen overlay is allowed only if it renders the loaded character portrait (or a RAM-backed image produced from the same `SYSTEX_Menu_ProfileNN`/profile renderer source) and its telemetry proves sane display size plus portrait-source linkage. The current text/rectangle/black-screen overlay is a false-positive scaffold and must not score as final cover success. User correction 2026-06-27: "a big black screen" is unacceptable; the product must have visual beauty, i.e. a visible loaded-character portrait or equivalently portrait-backed intentional surface, not a blank/dark cover.
