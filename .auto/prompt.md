...
- Existing cover target found: `/home/banon/er-extract/nuxe-menu-20260619-170932/menu/05_010_profileselect.gfx` contains `MENU_DummyProfileFace_01..10`; native wrapper `05_010_ProfileSelect` at dump `0x14081f7e0` maps to deobf `0x14081f6f0`. Current Part B spike replaces the suppressed `05_000_Title` job with this ProfileSelect cover and exposes `oracle_title_custom_cover_profile_select_*` telemetry.
...
2. Runtime-validate the ProfileSelect cover replacement: `05_000_Title` suppressed, `05_010_ProfileSelect` cover job built, `SYSTEX_Menu_ProfileNN`/dummy face visible, and gold still loads behind it. If it conflicts with title flow, fall back to authoring a minimal custom Scaleform target with the same dummy symbol.
