...
launch_form(s) if {
	regex.match(`(?i)(^|[[:space:];|&()])(bash|sh|zsh|dash|source|\.|python3?|exec|command)[[:space:]]+(-[^[:space:];|&()]+[[:space:]]+)*([^[:space:];|&()]*/)?(capture-er-frame\.sh|run-armament-icons-smoke\.sh|armament-icons-watch\.py|er-smoke-driver\.sh|run-golden-mount-trace\.sh|run-golden-observe\.sh|run-me3-product-smoke\.sh|run-product-continue-direct-probe\.sh|run-multi-save-load-proof\.sh|run-samechar-3x-threedll\.sh|run-samechar-3x-twodll\.sh|run-vanilla-continue-capture\.sh|run-vanilla-reload-agentdriven\.sh|run-vanilla-reload-fps\.sh|run-vanilla-trace-userdrive\.sh|launch-vanilla-offline\.sh|run-accept-byte-focus-probe\.sh|run-camera-smoke\.sh|run-control-no-ownload-probe\.sh|run-install-job-probe\.sh|run-menu-drive-probe\.sh|run-native-continue-probe\.sh|run-own-dispatch-treatment-probe\.sh|run-own-load-pump-probe\.sh|run-ownpump-verify-probe\.sh|run-pab-advance-probe\.sh|run-product-menu-load-probe\.sh|run-realload-headless\.sh|run-realload-onscreen\.sh|run-refactor-smoke-onscreen\.sh|run-refactor-smoke\.sh|run-save-trace-probe\.sh|run-savedir-fix-probe\.sh|run-watch-onscreen\.sh|run-profile-portrait-capture-probe\.sh|record-portrait-run\.sh|record-title-gfx-proof-wf\.sh|run-onscreen-capture-at\.sh|capture-er-at-marker\.sh|launch-golden-both-trace\.sh|launch-golden-confirm-trace\.sh|launch-golden-mount-trace\.sh|smoke-er-window-placement\.sh|me3_live_launch\.py|sq-repro-selfdrive-monitor\.py|y22i-windows-ab-probe\.py|y22i_bounded_probe\.py|record-er-boot-video\.py|vanilla-control-probe\.py)([[:space:];|&()]|$)`, s)
}

# UNCONDITIONAL launcher scripts, DIRECT command-position execution (./script or path/script).
launch_form(s) if {
	regex.match(`(?i)(^|[;|&()]|&&|\|\|)[[:space:]]*(\./)?([^[:space:];|&()]*/)?(capture-er-frame\.sh|run-armament-icons-smoke\.sh|armament-icons-watch\.py|er-smoke-driver\.sh|run-golden-mount-trace\.sh|run-golden-observe\.sh|run-me3-product-smoke\.sh|run-product-continue-direct-probe\.sh|run-multi-save-load-proof\.sh|run-samechar-3x-threedll\.sh|run-samechar-3x-twodll\.sh|run-vanilla-continue-capture\.sh|run-vanilla-reload-agentdriven\.sh|run-vanilla-reload-fps\.sh|run-vanilla-trace-userdrive\.sh|launch-vanilla-offline\.sh|run-accept-byte-focus-probe\.sh|run-camera-smoke\.sh|run-control-no-ownload-probe\.sh|run-install-job-probe\.sh|run-menu-drive-probe\.sh|run-native-continue-probe\.sh|run-own-dispatch-treatment-probe\.sh|run-own-load-pump-probe\.sh|run-ownpump-verify-probe\.sh|run-pab-advance-probe\.sh|run-product-menu-load-probe\.sh|run-realload-headless\.sh|run-realload-onscreen\.sh|run-refactor-smoke-onscreen\.sh|run-refactor-smoke\.sh|run-save-trace-probe\.sh|run-savedir-fix-probe\.sh|run-watch-onscreen\.sh|run-profile-portrait-capture-probe\.sh|record-portrait-run\.sh|record-title-gfx-proof-wf\.sh|run-onscreen-capture-at\.sh|capture-er-at-marker\.sh|launch-golden-both-trace\.sh|launch-golden-confirm-trace\.sh|launch-golden-mount-trace\.sh|smoke-er-window-placement\.sh|me3_live_launch\.py|sq-repro-selfdrive-monitor\.py|y22i-windows-ab-probe\.py|y22i_bounded_probe\.py|record-er-boot-video\.py|vanilla-control-probe\.py)([[:space:];|&()]|$)`, s)
}

# GENERAL future-launcher guard (fragility reducer, added 2026-07-24): so a FUTURE ER launcher script
# added to scripts/ cannot silently bypass the gate before someone remembers to extend the explicit
# allowlist. Matches a `scripts/`-path script (any path prefix / nested subdir) whose BASENAME is a
# launcher by naming convention:
#   * ends in `-smoke.sh`  (every repo `scripts/*-smoke.sh` is an ER launcher), OR
#   * is an armament-* launcher: a `(run|launch|capture|record)-...armament...` verb form OR a
#     `...armament...-watch.py` watcher.
# The armament arm is INTENTIONALLY narrower than "basename contains armament": a bare-`contains` clause
# would false-positive on the OFFLINE `scripts/armament-icons-pixel-oracle.py` diff oracle and the
# `scripts/frida/armament_*.js` instrument libraries (neither launches ER). Requiring a launcher-verb
# prefix or `-watch.py` suffix keeps those offline armament tools UN-gated while still catching future
# launchers like `run-armament-foo-smoke.sh` / `capture-armament-bar.py`. `scripts/steam-running.sh`,
# `scripts/check.sh`, and other non-`-smoke`/non-armament helpers do not match. Explicit entries above
# stay authoritative; this is an additional net, not a replacement. Interpreter-fed form:
launch_form(s) if {
	regex.match(`(?i)(^|[[:space:];|&()])(bash|sh|zsh|dash|source|\.|python3?|exec|command)[[:space:]]+(-[^[:space:];|&()]+[[:space:]]+)*([^[:space:];|&()]*/)?scripts/([^[:space:];|&()]*/)?([^[:space:];|&()/]*-smoke\.sh|(run|launch|capture|record)-[^[:space:];|&()/]*armament[^[:space:];|&()/]*|[^[:space:];|&()/]*armament[^[:space:];|&()/]*-watch\.py)([[:space:];|&()]|$)`, s)
}

# GENERAL future-launcher guard, DIRECT command-position execution (./scripts/... or scripts/...).
launch_form(s) if {
	regex.match(`(?i)(^|[;|&()]|&&|\|\|)[[:space:]]*(\./)?([^[:space:];|&()]*/)?scripts/([^[:space:];|&()]*/)?([^[:space:];|&()/]*-smoke\.sh|(run|launch|capture|record)-[^[:space:];|&()/]*armament[^[:space:];|&()/]*|[^[:space:];|&()/]*armament[^[:space:];|&()/]*-watch\.py)([[:space:];|&()]|$)`, s)
}
...
# GENERAL future-launcher arm (mirrors the shell-command general clause; coarse raw scan). Same
# armament-verb / -watch.py / -smoke.sh convention, so a future launcher smuggled through
# run_experiment/ctx code is caught without waiting for the explicit list to be extended.
code_launch(low) if {
	regex.match(`scripts/([^[:space:]"',;|&()]*/)?([^[:space:]"',;|&()/]*-smoke\.sh|(run|launch|capture|record)-[^[:space:]"',;|&()/]*armament[^[:space:]"',;|&()/]*|[^[:space:]"',;|&()/]*armament[^[:space:]"',;|&()/]*-watch\.py)`, low)
}

code_launch(low) if {
	regex.match(`(capture-er-frame\.sh|run-armament-icons-smoke\.sh|armament-icons-watch\.py|er-smoke-driver\.sh|run-golden-mount-trace\.sh|run-golden-observe\.sh|run-me3-product-smoke\.sh|run-product-continue-direct-probe\.sh|run-multi-save-load-proof\.sh|run-samechar-3x-threedll\.sh|run-samechar-3x-twodll\.sh|run-vanilla-continue-capture\.sh|run-vanilla-reload-agentdriven\.sh|run-vanilla-reload-fps\.sh|run-vanilla-trace-userdrive\.sh|launch-vanilla-offline\.sh|run-accept-byte-focus-probe\.sh|run-camera-smoke\.sh|run-control-no-ownload-probe\.sh|run-install-job-probe\.sh|run-menu-drive-probe\.sh|run-native-continue-probe\.sh|run-own-dispatch-treatment-probe\.sh|run-own-load-pump-probe\.sh|run-ownpump-verify-probe\.sh|run-pab-advance-probe\.sh|run-product-menu-load-probe\.sh|run-realload-headless\.sh|run-realload-onscreen\.sh|run-refactor-smoke-onscreen\.sh|run-refactor-smoke\.sh|run-save-trace-probe\.sh|run-savedir-fix-probe\.sh|run-watch-onscreen\.sh|run-profile-portrait-capture-probe\.sh|record-portrait-run\.sh|record-title-gfx-proof-wf\.sh|run-onscreen-capture-at\.sh|capture-er-at-marker\.sh|launch-golden-both-trace\.sh|launch-golden-confirm-trace\.sh|launch-golden-mount-trace\.sh|smoke-er-window-placement\.sh|me3_live_launch\.py|sq-repro-selfdrive-monitor\.py|y22i-windows-ab-probe\.py|y22i_bounded_probe\.py|record-er-boot-video\.py|vanilla-control-probe\.py|build_mushroom_blender_edit\.sh|route_a_mushroom_build_arm_sweep\.sh|install_mushroom_man\.py)`, low)
}
