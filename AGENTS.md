...
Standing user order (2026-07-24, supersedes the 2026-07-17 auto-launch permission): gate **all** Elden Ring, Blender, Windows app, game, current-desktop, ME3, runtime-probe, smoke-test, forced-crash, or input-capture launches on an explicit current-turn user request to say exactly `go`. Do not launch automatically just because the next proof step needs a run. Prepare the command/artifacts/readiness checks, state briefly what will launch and why, then stop and wait until the user says `go`. The `go` gate is additional to all existing forbidden Steam/EAC launch forms, save-safety rules, visual-oracle restrictions, and destructive/irreversible boundaries.
...
Steam MUST be running before every Elden Ring runtime probe. Verify with `scripts/steam-running.sh` (or a script that sources it), not a bare `pgrep -x steam`: on the current WSL+Windows setup, Steam may be the Windows `steam.exe` process and `pgrep -x steam` false-negatives even while Steam is actually up. If the WSL-aware helper says absent, ask the user to start Steam (interactive login) before launching any probe. The offline `eldenring.exe` Proton launch reuses Steam's environment (wineprefix, CWD, account/save-dir id); with Steam down the game still boots but in a different environment, so the DLL debug log lands elsewhere and Steam-dependent state degrades into a non-representative run (observed 2026-06-21: a run came back `cold_char_mount_phase=5` yet appended zero debug lines and the default level-9 character). `scripts/run-product-continue-direct-probe.sh` now fails closed in `preflight()` when the WSL-aware Steam helper says Steam is down.
