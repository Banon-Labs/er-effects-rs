...
For every agent-owned Elden Ring runtime probe/watchdog, teardown must be tied to all terminal and missing-progress states, not only PASS/explicit DERAILED/the hard cap. If harness logs/phases never appear promptly, if a phase/log stops advancing while the game remains open, or if the user reports they are left sitting in-game after the intended proof path stalled, immediately tear down the owned process tree, preserve artifacts, and fix the watcher teardown clause before another launch. Do not leave the game running until the long runtime cap merely because no success/failure semaphore fired. Parent-session ownership is mandatory: after launching a background runtime probe, actively poll job output/artifacts/process state on a short cadence until PASS/FAIL/cleanup is proven; do not rely on a detached job, a blocking/long `jobs attach`, long attach timeout, or future notification as evidence that teardown happened. For agent-owned Elden Ring probes, use short explicit poll steps (job output + artifact report/log mtimes + Windows process scan) instead of handing control to a long foreground wait. If the expected no-progress timeout is 12s or 30s, the parent must verify the run has exited or torn down shortly after that window and must kill the owned process tree itself if the watcher has not. If the user reports they manually quit, backed out, or otherwise had to intervene because the probe was too slow, treat that as a failed parent-orchestration proof even when the watcher eventually writes a failure verdict; record the artifact, tighten the parent poll/teardown path, and do not run another probe until the faster ownership path is validated without launching.
