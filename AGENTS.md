...
- The deobf binary and the dump share the SAME VAs (both 1.16.1; image base `0x140000000`), so a deobf VA looks up directly in the dump. Caveat: the dump's Ghidra function-boundary detection can differ -- a deobf VA may land mid-function (use `getFunctionContaining`, not `getFunctionAt`).
