...
- **Capitals in a comment name something; they never add emphasis** (user directive 2026-09-07,
  ...
  - The gate is a per-file ratchet over `scripts/comment-caps.baseline.json` (38,682 shouted words
    across 1,025 files when it landed, so it stops the growth rather than pretending the debt is
    gone). A file may not gain shouted words; a file that loses them must record the new number
    with `--update-baseline` in the same commit. A file with no baseline entry must have none at
    all, so new code is held to the rule from its first line. Burn one down with
    `scripts/check-comment-caps.py --fix <path>` and then read the diff -- a shouted word is
    occasionally load-bearing and wants a rewritten sentence, not a lowercase letter.
