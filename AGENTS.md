...
Standing user order (2026-07-19): the loading-bar progress oracle must be granular enough to reveal when a displayed terminal step such as 11/11 is not actually the final load/handoff step. If later RAM/native semaphores remain after the visible bar reaches its nominal final frame, fold those intermediate semaphores into the machine-readable loading-progress signature and report them as part of loading progress instead of treating the bar's nominal final frame as completion.
