...
Here "semaphore" means an **in-process memory-read telemetry oracle** -- a value the DLL derives by reading the game's PE/RAM (the `oracle_*` fields in `er-effects-telemetry.json`: e.g. `oracle_msgbox_total_builds`, `oracle_player_present`, `oracle_saved_map_c30`, `oracle_server_status_any_seen`, `oracle_char_name`), NOT a `bd` memory and NOT the screenshot. RAM-read oracles remain the run-stopping/product-proof detectors. However, for the current loading-screen-portrait proof of concept, the agent must inspect the event screenshot personally: until a correct native/pixel memory semaphore exists, visual evidence is the only way to tell whether the loading-screen portrait is actually working and visually acceptable.
