...
Every runtime/autoresearch probe **must** produce a low-quality teardown screenshot of the validated ER window before the game is killed: `scripts/capture-er-window.py` runs first in the probe's `cleanup()` trap (on `EXIT`/`INT`/`TERM`/`HUP`), writing `<ARTIFACT_DIR>/teardown-screenshot.jpg` (per run: `target/runtime-probe/<probe>-<timestamp>/teardown-screenshot.jpg`), or a `teardown-screenshot.txt` note only when the game/window is already gone or unmapped/hidden. Teardown capture is mandatory and focus-independent: the helper switches to/raises the exact `steam_app_1245620` window best-effort and does **not** fail merely because the window was not focused/topmost before teardown. Keep this wired into every probe harness; if a new harness is added, make the same capture run unconditionally on its teardown.
...
Screenshots/visual state must **never** be used as the run-stopping oracle or product-proof oracle. Stop/continue decisions must come from RAM/in-process telemetry semaphores, structured progress, explicit failure semaphores, process exit, or the hard runtime cap. Visual evidence is diagnostic only after a run (or user-provided "for posterity" context). During the current title-cover/autoload autoresearch, seeing the `LOAD GAME` / profile-select screen with no finished product is an expected unfinished state because it masks the old title menu; do not classify that view itself as a blocker, success, or unexpected condition unless a RAM semaphore says the underlying state is wrong.
