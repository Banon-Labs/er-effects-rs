...
All user-visible launches are user-gated. Do not launch Elden Ring, Blender, ME3, games, or other current-desktop/user-visible applications unless the user gives explicit current-turn approval for that exact launch class after the expected user-visible impact is stated. The launch-clearance keyphrase remains `LAUNCH CLEARANCE GRANTED` unless the user explicitly asks to change the executable policy. Previous standing runtime-validation orders do not override this gate. If a launch would be the next proof step, stop at the launch-ready artifact/profile and ask for the narrow launch gate instead of proceeding. This must be enforced by Cupcake/Rego policy for agent Bash launch paths; if policy coverage is missing or not loaded, fix that executable gate before attempting another launch. If the user suggests wording or behavior that appears to conflict with an existing policy, ask before updating the policy; do not infer policy-change authorization from an offhand suggestion.
...
If the user reports that they already tore down or closed a user-visible game/application run, treat that as authoritative cleanup ownership for that run. Do not run agent teardown/stop scripts after such a report unless the user asks you to run cleanup or a structured state check proves the game/application is still running and must be stopped before the next user-approved launch. In that case, state the still-running evidence first and stop only the specific leftover process/window needed for the next step.
