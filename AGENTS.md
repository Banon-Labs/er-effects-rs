...
Standing user order (2026-07-19): the loading-bar progress oracle and user-visible loading-bar label must be granular enough to reveal when a displayed terminal step such as 11/11 is not actually the final load/handoff step. The expected user-visible shape is `<text label> N/M (<sub milestone label> X/Y)`: the main `N/M` is the current visible/semantic loading label sequence, and the parenthesized sub-milestone `X/Y` exposes remaining hidden/native RAM semaphores within that label. Do not reinterpret this as merely extending a machine-only stall signature. If later RAM/native semaphores remain after the visible bar reaches its nominal final frame, either add them as visible main labels or surface them in the parenthesized sub-milestone count, and keep the machine-readable loading-progress signature aligned with those visible/sub-visible steps instead of treating the bar's nominal final frame as completion.
