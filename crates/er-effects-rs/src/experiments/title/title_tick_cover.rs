...
pub(crate) unsafe fn product_core_autoload_tick(module_base: usize, slot: i32, tick: u64) -> bool {
    ...
    // so a healthy load never trips it; clears 0x5d the frame mms leaves 18 (avoids the ~4s bounce).
    {
        ...
        let finalize_forcing_enabled = crate::experiments::gating::outgoing_teardown_enabled()
            && active_switch
    ...
    if slot >= OWN_STEPPER_SLOT_ZERO && gm != null {
        ...
        if SYSTEM_QUIT_QUICKLOAD_PHASE.load(Ordering::SeqCst)
            ...
            && mms_step == 18
            && !crate::experiments::gating::outgoing_teardown_suppresses_holds()
        {
        ...
        if let Some(md) = menudata {
            ...
            // DISABLED AGAIN -- LIVE-DISPROVEN AS A PARKED-STATE RECOVERY (2026-07-31, run
            // 20260731-user-portrait-verify-2). The 2026-07-31 re-enable (player_present + 600-frame
            // streak, aimed at the post-entry parked state that leaves the System windows hidden and
            // the Load Profile row dead) fired exactly as scoped -- and TORE THE WORLD DOWN: on a
            // PLAYABLE parked world the 0x5d write is an ending/return-title request, so the advancer
            // walked mms 18->Cleanup->Finish->-1 (player_present -> false, char gone, no reload armed)
            // and the user faded to black mid-gameplay into a dead post-teardown state. The 2026-07-18
            // teardown-the-OUTGOING-world usage and the 2026-07-20 mid-load completion are NOT this
            // state; do not re-enable this for a world that has already entered. The parked-finalize
            // problem (bd parked-finalize-mms18-dead-menus-rt5d-reenable-2026-07-31, AMENDED) needs a
            // real completion mechanism for the INCOMING step machine, not the ending request.
    ...
    if quickload_phase_for_b78_guard >= SYSTEM_QUIT_QUICKLOAD_PHASE_RETURN_TITLE_REQUESTED
        ...
        // switch #2 / cross-save Banon). Once the switch-reload fd4io machine has left IDLE it OWNS
        // GameMan+0xb78 as the warp target ("b78 kept armed (warp target) through finalize" at
        // SUBMIT->DRAIN->COMMIT); this guard's force-(-1) below raced that finalize when the
        // quickload phase lagged at 3 (+95422ms: "kept gm_b78=-1" 650ms AFTER the fd4io COMMIT) and
        // stripped the warp destination mid-warp -- STEP_MoveMap_Update reads BlockId 0xffffffff,
        // skips the map load, and the world tears down with nothing armed (black screen, defaulted
        // level-9 character). The in-world protection this guard exists for is unaffected: fd4io
        // SUBMIT only ever happens post-teardown at the clean title, so IDLE covers the whole
        // in-world window (the 2026-07-01 RequestLoadSlot spin). Reset per switch by
        // reset_switch_reload_latches.
        && er_telemetry::counters::SWITCH_RELOAD_FD4IO_PHASE.load(Ordering::SeqCst)
            == crate::experiments::SWITCH_RELOAD_FD4IO_IDLE
        && gm != null
