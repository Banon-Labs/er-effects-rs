...
pub(crate) unsafe extern "system" fn profile_row_populate_hook(
        ...
        && PROFILE_STATS_PUSH_IN_PROGRESS.swap(1, Ordering::SeqCst) == 0
    {
        ...
        if base != null {
            ...
            // PER-SLOT INFO FIELDS ON A BROWSE ROW (er-effects-rs-flkm). A browse row is a file or a
            // navigation entry, never a profile slot, so the record behind it is staged and its
            // numbers are zeros: the native fields render "Level 0" and "0:00:00" about a character
            // that does not exist. So while the picker owns a row, the Level caption and value are
            // always hidden -- there is no browse row they could be true of -- and the playtime slot
            // is repurposed: a save-FILE row shows when that file was last written, and every row
            // with no such time (navigation rows, and a file whose metadata was unreadable) hides
            // the field instead of showing a fabricated date.
            //
