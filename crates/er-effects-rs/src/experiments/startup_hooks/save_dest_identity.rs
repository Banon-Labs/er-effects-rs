...
pub(crate) struct SaveDestFileId {
    volume: u32,
    index: u64,
}

/// Result of asking the filesystem to identify a path.
pub(crate) enum SaveDestProbe {
...
#[derive(Clone, Copy, PartialEq, Eq)]
pub(crate) enum SaveDestIdentity {
...
pub(crate) fn save_dest_probe_file(path: &Path) -> SaveDestProbe {
...
pub(crate) fn save_dest_probe_dir(path: &Path) -> SaveDestProbe {
...
pub(crate) fn save_dest_file_identity(target: &Path, live: &Path) -> SaveDestIdentity {
...
pub(crate) fn save_dest_dir_identity(left: &Path, right: &Path) -> SaveDestIdentity {
...
pub(crate) fn save_dest_normalize_path(text: &str) -> Option<String> {
...
pub(crate) fn save_dest_normalized_parent(normalized: &str) -> Option<&str> {
...
pub(crate) fn save_dest_write_atomic(path: &Path, bytes: &[u8], tag: &str) -> Result<(), String> {
