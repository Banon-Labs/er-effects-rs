...
pub use hudhook::windows::Win32::Foundation::HINSTANCE;
use hudhook::{
...
impl EffectsState {
    fn new(mode: RuntimeDllMode) -> Self {
        let (calls, load_error) = if mode.effects_enabled() {
            match embedded_effects() {
                Ok(effects) => (
                    effects
                        .calls
                        .into_iter()
        ...
        Self {
            calls,
            effects_enabled: mode.effects_enabled(),
            load_error,
...
impl Default for EffectsState {
    fn default() -> Self {
        Self::new(RuntimeDllMode::Effects)
    }
}
...
/// # Safety
///
/// DINPUT8 proxy implementation. Export this from the effects DLL wrapper only;
/// the quickload DLL intentionally has no DINPUT8 proxy surface.
...
/// # Safety
///
/// Effects/HUD DLL entrypoint implementation. The cdylib wrapper exports this
/// as `DllMain` to keep `er_effects_rs.dll`'s public ABI stable.
pub unsafe extern "C" fn effects_dll_main(
    hmodule: HINSTANCE,
    reason: u32,
    reserved: *mut c_void,
) -> i32 {
    unsafe { dll_main_impl(hmodule, reason, reserved, RuntimeDllMode::Effects) }
}

/// # Safety
///
/// Quickload-only DLL entrypoint implementation. This initializes the autoload
/// runtime and telemetry without installing the HUD/effects overlay.
pub unsafe extern "C" fn quickload_dll_main(
    hmodule: HINSTANCE,
    reason: u32,
    reserved: *mut c_void,
) -> i32 {
    unsafe { dll_main_impl(hmodule, reason, reserved, RuntimeDllMode::Quickload) }
}

unsafe fn dll_main_impl(
    hmodule: HINSTANCE,
    reason: u32,
    ...
    let initial_state = EffectsState::new(mode);
    ...
    if mode == RuntimeDllMode::Quickload || autoload_without_overlay || overlay_disabled() {
        write_bootstrap_event(
