...
pub struct SavePickerModel {
    ...
    /// Separate keyboard/pad focus, transient pointer hover/ownership, observed native row, and
    /// transactionally committed pointer presence for the combined drive/path row.
    drive_strip_interaction: DriveStripInteractionState,
    ...
    /// Path text the user submitted that failed validation. While set, the CurrentPath control
    /// renders THIS text instead of `current_dir`, in the invalid colour, so a rejected entry stays
    /// on screen to be corrected rather than silently reverting. Cleared by `clear_status_message`,
    /// which every successful navigation already calls -- so entering a valid path returns the
    /// control to its normal colour without any separate bookkeeping.
...
impl SavePickerModel {
    ...
    pub fn cycle_drive_from_drive_strip(&mut self, forward: bool) -> bool {
        ...
        if changed {
            ...
            if let Some(cell) = self.drive_strip_active_cell() {
                let _ = self.set_drive_strip_focus(DriveStripFocus::Cell(cell));
            }
        }
        changed
    }
    ...
    pub fn set_current_dir_from_text(&mut self, text: &str) -> Result<bool, DirectoryChangeError> {
        ...
        // The submission is valid from here on, so clear any previous rejection BEFORE branching --
        // the old code only cleared on the same-directory branch, which left a rejected path (and
        // its invalid colouring) latched across a real navigation.
        self.clear_status_message();
    ...
    pub fn clear_status_message(&mut self) {
        ...
        // A rejected path is the visual half of the same rejection: clearing one must clear the
        // other, or a stale yellow path would follow the user into a folder they navigated to
        // successfully.
        self.rejected_path_text = None;
    }

    /// The path text the user submitted that failed validation, if any.
    pub fn rejected_path_text(&self) -> Option<&str> {
        self.rejected_path_text.as_deref()
    }

    /// Keep a rejected submission on screen instead of reverting to `current_dir`.
