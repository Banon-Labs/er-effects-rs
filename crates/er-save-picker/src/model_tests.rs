// Unit tests for `model.rs`, split into a `#[path]` child so `model.rs` stays under the repo's
// hard file-size limit. `use super::*` still resolves to the model module.
...
#[test]
/// The CurrentPath control's submit contract, per the 2026-08-11 request: a valid path
/// navigates, an invalid one keeps the typed text on the control marked invalid instead of
/// silently reverting, and a subsequent valid entry clears that marking.
#[test]
fn a_rejected_path_stays_on_the_control_until_a_valid_one_replaces_it() {
    let dir = std::env::temp_dir();
    let mut model = SavePickerModel::default();

    // Nothing rejected yet: the control renders the current directory.
    assert_eq!(model.rejected_path_text(), None);

    // Invalid submission: the directory is unchanged and the typed text is retained so the user
    // can correct it rather than retyping from scratch.
    let before = model.current_dir().to_path_buf();
    let bogus = "Z:\\definitely\\not\\a\\real\\folder\\ateb";
    assert!(model.set_current_dir_from_text(bogus).is_err());
    model.set_rejected_path_text(bogus);
    assert_eq!(model.rejected_path_text(), Some(bogus));
    assert_eq!(model.current_dir(), before.as_path());

    // A valid submission navigates AND clears the rejection, so the control returns to its
    // normal colour without any separate reset step.
    let valid = dir.to_str().expect("temp dir is utf-8");
    assert!(model.set_current_dir_from_text(valid).is_ok());
    assert_eq!(
        model.rejected_path_text(),
        None,
        "a successful navigation must clear the invalid marking"
    );
}
