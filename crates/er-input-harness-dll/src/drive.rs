...
/// A load completing to menu-drive readiness: the direct input manager must be present before the
/// next phase may open the pause menu. `now_loading` is deliberately not required to clear here: live
/// artifacts on 2026-07-24 showed `world_sim=1, load_fsm=0, now_loading=1` while the user was already
/// effectively in game. The menu-drive prerequisite is input availability, not that stale latch.
...
impl Phase {
    ...
    fn tick(self, base: usize, im: usize, frame: u64, sem: &Sem) -> Status {
        let advanced = match self {
            ...
            Phase::WaitLoadIn => sem.world_sim && sem.load_fsm == 0 && im != 0,
            Phase::OpenPauseMenu => {
                if !pause_menu_open() {
                    request_open_ingame_menu(im);
                }
