...
//! only in part):
//! * `rows` -- `startup_hooks/system_quit_row_identity.rs` (921, ~100% product): the
...
//! * `dialog_handlers` -- `startup_hooks/system_quit_dialog_handlers.rs` (1601): the
...
//! * `browse` -- `startup_hooks/save_picker_menu.rs` (944): the in-game
...
//! * `surface` -- `startup_hooks/save_picker_surface.rs` (222): THE one place that decides
//!   which picker surface opens, plus the destination decisions both surfaces share.
//! * `dim` -- `startup_hooks/save_picker_dim_overlay.rs` (876): the layered GDI window
...
//!   `startup_hooks/save_picker_os_dialog.rs` (`os_open_save_picker_load`,
...
//!   `startup_hooks/save_swap_profile_table.rs` (1050): the foreign-save ProfileSummary
...
//!   `startup_hooks/layout_global_hooks.rs`: the single entry point that installs the
//!   whole feature.
//!
