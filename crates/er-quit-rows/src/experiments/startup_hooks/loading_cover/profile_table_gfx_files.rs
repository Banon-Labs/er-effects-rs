...
pub(crate) unsafe extern "system" fn title_scaleform_file_open_observer_hook(
    ...
    let ret = if base != null {
        if orig != null && orig != HOOK_ORIGINAL_UNSET {
            ...
            //
            // 1. `05_000_title`, the stripped title screen. The title is not a Quit row surface.
            ...
            // }
            //
            // 2. `02_990_TextInput_BuildUrl`, the link field. Its row left with `build-rows`, so
