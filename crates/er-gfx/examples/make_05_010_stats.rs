...
//! - REMOVE the 128x128 face box placement (`Icon_0`, char 66) at (-448,0) to
//!   free the row's left strip (user direction 2026-07-04). The native
//!   row-populate's `Icon_0` setters are dataType-guarded (RE: FUN_140749a80 /
//!   FUN_14074bdb0 both bail unless the member resolves to a display object),
//!   so an unplaced `Icon_0` is a safe native no-op.
//! - REPURPOSE char 67 (the icon frame deco sprite, only placed here) as a new
//!   `DefineEditText` stats field, placed as `ErStats` at (-300,15), 630x40,
//!   left-aligned, 22px `MenuFont_01`. The name matches no engine populate
//!   prefix (StaticText_/StaticRegionText_/StaticLineHelp_/StaticSystemText_/
//!   StaticDialogText_/StaticKeyGuide_/Dynamic+KeyIcon_), so only our DLL push
//!   writes it.
//! - MOVE PlayerName (-354,-48)->(-512,-48), the Level FMG caption
//!   StaticText_110502 (-354,15)->(-512,15), and the Level value field
//!   (-58,15)->(-364,15) into the freed strip; Location and PlayTime keep
//!   their native placements.

use er_gfx::title_05_010::STATS_FIELD_NAME;
use er_gfx::{Matrix, Movie, Rect, Tag};
...
fn main() {
    ...
    *deco = stats_field;
    ...
    // Remove the face box placement.
    let icon = row
        .iter()
        .position(|t| name_of(t).as_deref() == Some("Icon_0"))
    ...
    *deco_place = Tag::PlaceObject2 {
        ...
        ratio: None,
        name: Some(STATS_FIELD_NAME.to_owned()),
        clip_depth: None,
        force_long: false,
    };

    // Shift the left-column fields into the freed strip.
    for (name, x, y) in [
        ("PlayerName", -512, -48),
        ("StaticText_110502", -512, 15),
        ("Level", -364, 15),
    ] {
        let tag = row
            .iter_mut()
