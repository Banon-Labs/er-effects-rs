...
impl BoneName {
    ...
    fn display(&self) -> String {
        if let Some(name) = self.as_ascii() {
            name.to_owned()
        } else {
    ...
    for (vertex_index, vertex) in mesh.vertices.iter().enumerate() {
        for slot in 0..4 {
            ...
            let bone_name = mesh
                ...
                .map(|bone| &bone.name);
            let bone_name_display = bone_name
                .map(BoneName::display)
                .unwrap_or_else(|| "<missing>".to_owned());
            let target_bone = bone_name
                .map(er_target_for_source_bone)
                .unwrap_or("<missing>");
            writeln!(
                file,
...
fn write_summary(path: &Path, config: &Config, mesh: &ExportedMesh) -> io::Result<()> {
    ...
    writeln!(
        file,
        "scaled_height={:.9}",
        (mesh.header.bbox_max.y - mesh.header.bbox_min.y) * config.scale
    )?;
...
fn er_target_for_source_bone(source: &BoneName) -> &'static str {
    match source.as_ascii().unwrap_or("") {
        "Pelvis" => "Pelvis",
        "Spine1" => "Spine",
        "Spine2" => "Spine1",
        "Spine3" => "Spine2",
...
fn read_cstring_name(bytes: &[u8], offset: usize) -> BoneName {
    if offset >= bytes.len() {
        return BoneName { bytes: Vec::new() };
    }
