...
fail() { echo "run-samechar-3x: $*" >&2; exit 2; }
...
{
  echo 'profileVersion = "v1"'; echo
  echo '[[supports]]'; echo 'game = "eldenring"'; echo
  echo '[[natives]]'; echo "path = '$(win_path "$PRODUCT_GAMEDIR")'"; echo
  echo '[[natives]]'; echo "path = '$(win_path "$TRACE_GAMEDIR")'"
} > "$PROFILE"
...
{ echo "# staged by run-samechar-3x-twodll.sh"; echo "save_file = '$(win_path "$BOOT_FILE")'"; echo "slot = $BOOT_SLOT"; } > "$GAME_DIR/er-effects.toml"
...
rm -f "$GAME_DIR/er-effects-switch-slot.txt" 2>/dev/null
