...
blocked(command, oids) if { count(guard.deny with input as event(command, oids)) == 1 }
...
test_allow_fresh_guarded_commands if { count(guard.deny with input as event("git rebase origin/main", same)) == 0; count(guard.deny with input as event("git push --force origin feature/x", same)) == 0 }
test_allow_unrelated_commands_when_stale if { count(guard.deny with input as event("git rebase origin/release", stale)) == 0; count(guard.deny with input as event("git push origin feature/x", stale)) == 0; count(guard.deny with input as event("printf 'git push --force'", stale)) == 0 }
"""
