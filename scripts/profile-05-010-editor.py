...
def validate_data(data, selected=None):
    ...
    drive0 = data.get("fields", {}).get("DriveCell_0", {})
    drive1 = data.get("fields", {}).get("DriveCell_1", {})
    drive2 = data.get("fields", {}).get("DriveCell_2", {})
    path = data.get("fields", {}).get("CurrentPath", {})
    try:
        pitch = float(drive1.get("x", 0)) - float(drive0.get("x", 0))
        if pitch <= 0 or abs((float(drive2.get("x", 0)) - float(drive1.get("x", 0))) - pitch) > 1e-6:
            errors.append("drive cells must use one positive pitch")
        if any(float(field.get("width", 0)) > pitch for field in (drive0, drive1, drive2)):
            errors.append("drive cell width must fit within the drive pitch")
        if float(drive2.get("x", 0)) + float(drive2.get("width", 0)) + 4 > float(path.get("x", 0)):
            errors.append("CurrentPath must leave a 4px gutter after the authored drive controls")
        if float(path.get("x", 0)) + float(path.get("width", 0)) > -float(drive0.get("x", 0)):
            errors.append("CurrentPath right edge exceeds the normal drive-row margin")
    except (TypeError, ValueError):
        errors.append("drive/path row composition has non-numeric geometry")
...
function draw(){ layoutHealth(); const svg=document.getElementById('canvas'); svg.innerHTML=''; if(!(Number(data.list.row_pitch)>0)) { document.getElementById('layoutHealth').textContent='list.row_pitch must be > 0; canvas not drawn'; return; } const rowPitch=data.list.row_pitch, half=data.list.mask_height/2, showClips=document.getElementById('showClips').checked, clipPreview=document.getElementById('clipPreview').checked; const defs=add('defs',{},svg); for(let y=-half;y<=half;y+=rowPitch) add('line',{x1:-620,x2:640,y1:y,y2:y,class:'rowline'}); add('rect',{x:-620,y:-half,width:1260,height:data.list.mask_height,fill:'none',stroke:'#777','stroke-dasharray':'6 4'}); add('rect',{x:data.list.scrollbar_x,y:data.list.scrollbar_top_y,width:10,height:data.list.scrollbar_track_height,fill:'#8884',stroke:'#aaa'});
 ...
 if(!driveMode){ const backing=data.row_chrome.backing; const bx=backing.x-27*backing.scale_x, by=backing.y-27*backing.scale_y, bw=56*backing.scale_x, bh=54*backing.scale_y; r=add('rect',{x:bx,y:by,width:bw,height:bh,class:'chrome',stroke:selected.kind=='chrome'&&selected.name=='backing'?'#fff':'#ffbd5a','stroke-width':selected.kind=='chrome'&&selected.name=='backing'?2:1}); bindDrag(r,'chrome','backing'); add('text',{x:bx,y:by-4,class:'label'}).textContent='normal backing char54/shape53';
 const c=data.row_chrome.cursor; const cb=data.row_chrome.cursor_body; const cx=c.x+cb.x*c.scale_x-27*c.scale_x*cb.scale_x, cy=c.y+cb.y*c.scale_y-27*c.scale_y*cb.scale_y, cw=56*c.scale_x*cb.scale_x, ch=54*c.scale_y*cb.scale_y; r=add('rect',{x:cx,y:cy,width:cw,height:ch,class:'chrome',stroke:selected.kind=='chrome'&&selected.name=='cursor'?'#fff':'#ffbd5a','stroke-width':selected.kind=='chrome'&&selected.name=='cursor'?2:1}); bindDrag(r,'chrome','cursor'); add('text',{x:cx,y:cy-25,class:'label'}).textContent='effective highlight body (cursor + inner body)';
 const body=data.row_chrome.cursor_body; add('rect',{x:body.x-27*body.scale_x,y:body.y-27*body.scale_y,width:56*body.scale_x,height:54*body.scale_y,class:'readonly'}); add('text',{x:body.x,y:body.y-25,class:'label'}).textContent='raw inner highlight body only'; }
 if(driveMode){ const button=data.row_chrome.drive_button; const selectedCell=selected.kind=='field'&&selected.name.startsWith('DriveCell_')?Math.max(0,Math.min(2,Number(selected.name.slice(10))||0)):0; for(let i=0;i<3;i++){ const f=data.fields['DriveCell_'+i]; const bx=Number(f.x)+Number(button.x), by=Number(f.y)-2+Number(button.y), bw=Number(f.width)*Number(button.scale_x), bh=Number(f.clip_height)*Number(button.scale_y); r=add('rect',{x:bx,y:by,width:bw,height:bh,class:'chrome',stroke:selected.kind=='chrome'&&selected.name=='drive_button'?'#fff':'#ffbd5a','stroke-width':selected.kind=='chrome'&&selected.name=='drive_button'?2:1}); bindDrag(r,'chrome','drive_button'); if(i==selectedCell){ add('rect',{x:bx,y:by,width:bw,height:bh,class:'readonly',stroke:'#62e6ff','stroke-width':2}); add('text',{x:bx,y:by-5,class:'label'}).textContent='active native Cursor preview'; } } const pf=data.fields.CurrentPath, pbx=Number(pf.x)+Number(button.x), pby=Number(pf.y)-2+Number(button.y), pbw=Number(pf.width)*Number(button.scale_x), pbh=Number(pf.clip_height)*Number(button.scale_y); r=add('rect',{x:pbx,y:pby,width:pbw,height:pbh,class:'chrome',stroke:selected.kind=='field'&&selected.name=='CurrentPath'?'#fff':'#9bd0ff','stroke-width':selected.kind=='field'&&selected.name=='CurrentPath'?2:1}); bindDrag(r,'field','CurrentPath'); add('text',{x:pbx,y:pby-5,class:'label'}).textContent='clickable full-path control'; add('text',{x:-535,y:-24,class:'label'}).textContent='drive row: active Cursor uses the selected cell button bounds'; } }
...
def self_test():
    ...
    oversized = json.loads(json.dumps(data))
    oversized["fields"]["CurrentPath"]["width"] = 700
    oversized_errors = validate_data(oversized)
    if not any("normal drive-row margin" in error for error in oversized_errors):
        raise SystemExit("oversized CurrentPath did not fail the drive-row margin contract")
