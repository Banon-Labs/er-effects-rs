...
# The asset and the edit table are REGENERATED by this point. Everything below is verification, so
# a failure here must not read as "the rebuild did not happen" -- it did.
#
# This used to be `cargo fmt --all -- --check`, which fails on ANY unformatted file in the
# workspace, including files this script never touches. Under `set -e` that aborted the run after a
# successful regeneration, with the generated asset already on disk and the only visible output a
# rustfmt diff for an unrelated crate. Observed 2026-08-07 while another agent had er-loading-portrait
# mid-edit; it looked exactly like the generator being broken or non-deterministic, and cost a
# round of re-running and hashing to disprove.
#
# So: FORMAT what this script generates (er-gfx owns both generated files), then report the
# workspace state as advisory rather than fatal.
cargo fmt -p er-gfx
if ! cargo fmt --all -- --check > /dev/null 2>&1; then
  echo "rebuild-profile-05-010-layout: NOTE -- other workspace files are unformatted (not this script's output)." >&2
  echo "rebuild-profile-05-010-layout: the asset and edit table above were regenerated successfully; run 'cargo fmt --all' before committing." >&2
fi
