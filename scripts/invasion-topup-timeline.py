...
def main(argv: list[str]) -> int:
    ...
    if walks:
        ...
        if all(w.startswith("0 ") for w in walks) and marks:
            print(
                "  EVERY repaint pass walked ZERO claimed dormant rows. If this run had a top-up, "
                "its markers were NOT re-coloured and marking them cannot show on screen."
            )
    elif repaints:
        print(
