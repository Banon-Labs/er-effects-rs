...
const LIMITS = {
	rowTransform: 200,
	insertItem: 120,
	getReinforcement: 120,
};

const counts = {};
let activeGiveAttempted = false;
...
function writeEntry(entry, itemId, quantity, upgrade, gemId) {
	entry.add(OFFSETS.itemLotItemId).writeS32(itemId);
	entry.add(OFFSETS.itemLotQuantity).writeS32(quantity);
	entry.add(OFFSETS.itemLotUpgrade).writeS32(upgrade);
	entry.add(OFFSETS.itemLotGemId).writeS32(gemId);
}

function makeSeedList() {
	const listSize = OFFSETS.itemLotItems + 10 * OFFSETS.itemLotEntrySize;
	const list = Memory.alloc(listSize);
	Memory.writeByteArray(list, new Uint8Array(listSize));
	list.add(OFFSETS.itemLotCount).writeU32(2);
	writeEntry(list.add(OFFSETS.itemLotItems), ITEMS.dagger, 1, 0, ITEMS.noGem);
	writeEntry(
		list.add(OFFSETS.itemLotItems + OFFSETS.itemLotEntrySize),
		ITEMS.smithingStone1RawGoods,
		10,
		-1,
		ITEMS.noGem,
	);
...
function tryActiveGive() {
	if (activeGiveAttempted) return;
	activeGiveAttempted = true;

	const mapItemMan = safePtr(ptrAt(RVAS.mapItemManGlobal));
	const list = makeSeedList();
	const status = Memory.alloc(12);
	Memory.writeByteArray(status, new Uint8Array(12));
	emit("active_give_before", {
		map_item_man: hexPtr(mapItemMan),
		item_lot: readItemLot(list),
		current_open_menu: currentOpenMenu(),
	});
	if (mapItemMan === null || mapItemMan.isNull()) {
		emit("active_give_skip", { reason: "missing_map_item_man" });
		return;
	}
	try {
		const giveItems = new NativeFunction(ptrAt(RVAS.giveItems), "void", [
			"pointer",
			"pointer",
			"pointer",
			"uint32",
		]);
		giveItems(mapItemMan, list, status, 0);
		emit("active_give_after", {
			status0: status.readU32(),
			status1: status.add(4).readU32(),
			status2: status.add(8).readU32(),
			current_open_menu: currentOpenMenu(),
		});
	} catch (error) {
		emit("active_give_error", {
			error: String(error),
			current_open_menu: currentOpenMenu(),
		});
	}
...
emit("ready", { current_open_menu: currentOpenMenu(), mode: "give_on_open" });
