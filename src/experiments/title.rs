...
pub(crate) unsafe fn maybe_bind_title_logo_resource_to_systex_cover(
    base: usize,
    ready: &ProductCoreAutoloadReady,
) {
    ...
    let renderer0 = unsafe { safe_read_usize(base + ACTIVE_SCREEN_ARRAY_RVA) }.unwrap_or(0);
    if renderer0 == 0 || renderer0 == TITLE_OWNER_SCAN_START_ADDRESS {
        return;
    }
    ...
    TITLE_CUSTOM_COVER_LOGO_REMAP_RENDERER_NONNULL.store(renderer0, Ordering::SeqCst);
    let set_visible: unsafe extern "system" fn(usize, u8) =
        unsafe { std::mem::transmute(base + TITLE_LOGO_BACK_VIEW_PARTS_SET_VISIBLE_RVA) };
    unsafe { set_visible(logo, 1) };
    append_autoload_debug(format_args!(
        "title-cover-part-b: remapped TitleBackViewParts MenuResource 0x{logo_resource:x}: {TITLE_CUSTOM_COVER_TITLE_IMAGE_SYMBOL} -> {TITLE_CUSTOM_COVER_SYSTEX_TARGET} through 0x{:x} dialog=0x{:x} logo=0x{logo:x} renderer0=0x{renderer0:x}; re-showed logo surface as profile cover",
...
pub(crate) unsafe fn product_core_autoload_tick(module_base: usize, slot: i32, tick: u64) -> bool {
    ...
    if phase == OWN_STEPPER_PHASE_MENU {
        ...
        if ready.menu_opened_latch == OWN_STEPPER_MENU_OPENED_NO {
            ...
            unsafe { maybe_bind_title_logo_resource_to_systex_cover(module_base, &ready) };
