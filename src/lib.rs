...
pub unsafe extern "C" fn DllMain(hmodule: HINSTANCE, reason: u32, _reserved: *mut c_void) -> i32 {
    ...
    if title_native_menu_visual_suppression_enabled() {
        ...
        START_TITLE_LOGO_BACK_VIEW_PARTS_FADEIN_SUPPRESS.call_once(|| {
            let _ = std::thread::Builder::new()
                .name("er-effects-title-logo-fadein".to_owned())
                .spawn(install_title_logo_back_view_parts_fadein_suppression_hook);
        });
    }
