...
pub(crate) fn write_oracle_telemetry(body: &mut String) {
    ...
    if let Ok(base) = crate::experiments::game_module_base() {
        ...
        let title_custom_cover_logo_remap_last_resource =
            TITLE_CUSTOM_COVER_LOGO_REMAP_LAST_RESOURCE.load(Ordering::SeqCst);
        let title_custom_cover_logo_remap_renderer_nonnull =
            TITLE_CUSTOM_COVER_LOGO_REMAP_RENDERER_NONNULL.load(Ordering::SeqCst);
        let title_profile_cover_bound_to_logo_surface = title_custom_cover_logo_remap_calls != 0
            && TITLE_CUSTOM_COVER_PROFILE_RENDER_REFRESH_CALLS.load(Ordering::SeqCst) != 0;
        ...
        body.push_str(&format!(
            ...
            title_custom_cover_logo_remap_last_resource,
            title_custom_cover_logo_remap_renderer_nonnull,
